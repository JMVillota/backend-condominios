...
const getAllAlquiler = (request, response) => {
    ...
    db.query('SELECT * FROM alquiler', (error, results) => {
        if (error)
            throw error
...
const getAlquilerById = (request, response) => {
    ...
    db.query('SELECT * FROM alquiler WHERE alq_id = $1', [alq_id], (error, results) => {
        if (error) {
            throw error
        }
        response.status(200).json(results.rows)
...
const deleteAlquiler = (request, response) => {
    ...
    const alq_id = request.params.alq_id;
    console.log('id is ' + alq_id)
    db.query('DELETE from alquiler WHERE alq_id=$1', [alq_id], (error, results) => {
        if (error)
            throw error

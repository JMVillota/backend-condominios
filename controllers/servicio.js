...
const createServicio = (request, response) => {
    ...
    db.query('INSERT INTO servicio (ser_fecha, ser_descripcion, ser_total, tser_id ) VALUES ($1,$2,$3,$4)', [ser_fecha, ser_descripcion, ser_total, tser_id], (error, results) => {
        if (error) {
            throw error
        }
...
const getServicioById = (request, response) => {
    ...
    db.query('SELECT * FROM servicio WHERE ser_id = $1', [ser_id], (error, results) => {
        if (error) {
            throw error
        }
...
const getAllServicios = (request, response) => {
    ...
    db.query('select * from servicio ', (error, results) => {
        if (error)
            throw error
        response.status(200).json(results.rows)
    })
...
const deleteServicio = (request, response) => {
    ...
    db.query('delete from servicio where ser_id=$1', [ser_id], (error, results) => {
        if (error)
            throw error
...
const updateServicio = (request, response) => {
    ...
    db.query('update servicio set ser_fecha=$1, ser_descripcion=$2, ser_total=$3, tser_id=$4 where ser_id=$5', [ser_fecha, ser_descripcion, ser_total, tser_id, ser_id], (error, results) => {
        if (error) {
            throw error
        }

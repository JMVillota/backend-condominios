...
const createDepartamento= (request, response) => {
    ...
    db.query(`INSERT INTO gest_adm_departamento (dep_id,res_id,dep_telefono,dep_estado,dep_ocupacion) 
...
const updateDepartamento = (request, response) => {
    ...
    db.query(`UPDATE gest_adm_departamento SET res_id=$1, dep_telefono=$2, dep_estado=$3, dep_ocupacion=$4 WHERE dep_id=$5`, [res_id,dep_telefono,dep_estado,dep_ocupacion,dep_id], (error, results) => {
        if (error) {
            throw error
            console.log(error)
        }
